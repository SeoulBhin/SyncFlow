...
| `.docs/UI.md` | 프론트 UI 기능 목록, **Mock 데이터 → API 전환 가이드** |
...
- **Slack 정렬 로드맵:** `.docs/UI.md` 끝 "Slack 정렬 로드맵" 섹션 참조 — Phase 0~3 + 도메인별 매트릭스. 새 작업은 항상 그 표에서 다음 카드를 뽑는다.
...
| `.docs/UI.md` | 프론트 UI 기능 전체 목록, **Mock → API 전환 가이드** | 프론트-백엔드 연동 시 |
