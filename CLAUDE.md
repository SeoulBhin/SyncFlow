...
- **백엔드**: 14개 모듈 폴더 생성. **`auth`(JWT + Google/GitHub/Kakao OAuth)·`settings`(테마/알림/비밀번호) 구현 완료.** 나머지 12개 모듈은 폴더만 있고 미착수.
...
### Backend (backend/src/)
- **NestJS 11 + TypeScript** — 14개 모듈로 분리 (`common`, `auth`, `settings`, `groups`, `projects`, `pages`, `tasks`, `schedules`, `channels`, `messages`, `ai`, `voice-chat`, `screen-share`, `subscriptions`)
- **현재 구현 완료:** `auth`(JWT + Google/GitHub/Kakao OAuth, 회원가입, 토큰 갱신, 비밀번호 재설정, 프로필) · `settings`(테마, 알림, 비밀번호 변경)
- **DB:** TypeORM + PostgreSQL 15+ (pgvector for AI embeddings)
- **캐싱:** Redis (ioredis)
- **인증:** JWT + Passport (Google/GitHub/Kakao OAuth)
- **코드 실행:** Docker 샌드박스 (Dockerode)
- **AI:** Google Gemini API + RAG 파이프라인 (pgvector 768-dim)
- **음성:** LiveKit Server SDK
