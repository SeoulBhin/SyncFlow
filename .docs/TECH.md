...
| TailwindCSS | 스타일링 | v4 |
| Zustand | 상태 관리 | 5, 13개 store (useThreadStore, useCustomFieldStore 추가) |
| React Router | 라우팅 | v7 |
...
### 5.6 외부 조직 공유 채널 (Slack Connect)

| 요소 | 사양 |
|------|------|
| 데이터 모델 | channels.is_external, channels.connected_org_ids[], members.org_id/org_name |
| 채널 헤더 | Globe 아이콘 + 연결 조직 배지 + 초대 버튼 |
| 공유 채널 배너 | 조직 목록 + 권한 표시 + 설정 아이콘 |
| 초대 모달 | 3단계 (이메일 입력 → 확인 → 전송 완료), 권한 4항목 체크 |
| 멤버 패널 | 외부 채널 시 조직별 그룹핑, 외부 멤버 아바타 오렌지 계열 |
| 메시지 표시 | 외부 멤버 메시지에 소속 조직 배지 |
| 권한 모델 | 외부 멤버는 guest 역할, 메시지/파일/멘션/앱 개별 제어 가능 |

### 5.7 문서 블록 시스템

| 요소 | 사양 |
|------|------|
| 슬래시 커맨드 | `/` 입력 시 13종 블록 타입 드롭다운 (키보드 네비게이션, 필터링) |
| 콜아웃 블록 | TipTap 커스텀 Node — info/warning/success/error 4종, 아이콘+배경색 |
| 토글 블록 | TipTap 커스텀 Node — 접기/펼치기, chevron 아이콘 애니메이션 |
| 블록 타입 | heading(3종), list(2종), task, callout, toggle, code, quote, divider, table, image |
| 회의록 템플릿 | 안건/논의/결정/액션아이템 구조화 HTML 상수, 1-클릭 적용 |

### 5.7 AI 통합 포인트

| 요소 | 사양 |
|------|------|
| Cmd+K AI | 검색 입력이 `?`로 끝나거나 한국어 질문 패턴 시 "AI에게 질문하기" 1순위 표시 → AI 패널 열림 |
| @AI 채널 멘션 | 메시지에 `@AI` 입력 시 1.5초 후 mock AI 봇 응답 삽입 |
| 제안 칩 | AI 패널 하단 입력 위에 라우트 기반 제안 ("이 문서 요약해줘", "마감 임박 작업 정리해줘" 등) |
| 컨텍스트 배너 | 회의 종료 시 "회의록 생성할까요?" 배너, 닫기 가능 |
...
| 역할 | 권한 |
|------|------|
| Owner | 전체 관리, 조직 삭제, 소유권 이전, **모든 그룹·프로젝트 자동 열람** |
| Admin | 멤버 관리, 채널 관리, 설정 변경, AI 무료, **모든 그룹·프로젝트 자동 열람** |
| Member | 참여한 그룹의 문서/코드/작업/메신저/음성만 접근 |
| Guest | 초대된 외부 공유 채널만 접근, 읽기 중심 |

**그룹 가시성**

| 속성 | 동작 |
|------|------|
| `visibility: 'public'` | 조직 내 누구나 검색·자유 참여 가능 (기본값) |
| `visibility: 'private'` | 초대된 멤버만 접근. 검색에 표시되지 않음 |
| `is_external: true` | 외부 조직과 공유. Guest 역할로 참여 |

**프로젝트 접근**

| 접근 경로 | 설명 |
|-----------|------|
| 그룹 멤버 | 그룹에 참여하면 해당 그룹의 모든 프로젝트 접근 |
| project_members | 그룹 미참여자도 Viewer/Member로 개별 초대 가능 |
| 조직 Owner/Admin | 모든 프로젝트 자동 열람 (전사 대시보드) |

**채널별 오버라이드**: `channel_members.permission_override` JSONB로 채널 단위 세부 권한 조정
