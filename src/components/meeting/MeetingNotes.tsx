      ...
      {notes.length > 0 && (
        <div>
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <h3 className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">AI 요약</h3>
          </div>
          {notes.map((note, i) => (
            ...
            {actionItems.map((item) => (
              ...
              >
                {item.done ? (
                  ...
                  <Circle size={14} className="mt-0.5 shrink-0 text-neutral-300 dark:text-neutral-600" />
                )}
                <div>
                  <p
                    className={`text-xs ${item.done ? 'text-neutral-400 line-through' : 'text-neutral-700 dark:text-neutral-200'}`}
                  >
                    {item.title}
                  </p>
                  <p className="text-[10px] text-neutral-400">{item.assignee}</p>
                </div>
              </div>
            ))}
          </div>
