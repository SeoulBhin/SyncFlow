...
export function GroupSettingsModal({ isOpen, onClose, groupName, inviteCode }: Props) {
  ...
  const handleSave = () => {
    if (!name.trim()) { addToast('error', '채널명을 입력해주세요.'); return }
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 rounded-lg bg-neutral-50 p-3 dark:bg-neutral-800/50">
              <span className="flex-1 text-center text-lg font-bold tracking-[0.25em] text-primary-600 dark:text-primary-400">{code}</span>
          ...
          <div className="rounded-lg border border-red-200 p-4 dark:border-red-800/50">
            ...
            <p className="mb-3 text-xs text-neutral-500 dark:text-neutral-400">채널을 삭제하면 모든 프로젝트와 페이지가 삭제됩니다.</p>
            {showDeleteConfirm ? (
              ...
              <Button variant="danger" size="sm" onClick={() => setShowDeleteConfirm(true)}><Trash2 size={14} />채널 삭제</Button>
            )}
          </div>
